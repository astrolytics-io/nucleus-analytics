...
const Nucleus = {
  ...
  track: debounce((eventName, data = undefined, type = 'event') => {
    ...
    if (type && ['init', 'error'].includes(type)) {
      ...
      Object.keys(extra).forEach((key) => { eventData[key] = extra[key] }) // Arrow function should not return assignment.
    }
...
const autoDetectData = () => {
  ...
  const undetectedProps = Object.keys(localData).filter(prop => !localData[prop])
